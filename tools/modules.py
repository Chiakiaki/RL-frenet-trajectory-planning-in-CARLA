...
from tools.utility import xy2lms, lms2xy,raw_data2points
    ...
    def _parse_events(self):
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                elif event.key == K_0:#add here, visulize the lidar
                    self.vis_lidar = True
...
class ModuleWorld:
    ...
    def _spawn_hero(self):
        ...
        # lidar sensor
        self.lidar_sensor = LidarSensor(self.hero_actor)
    ...
    def render(self, display):
        if self.args.play_mode == 2:
            if self.module_input.vis_lidar == False:
...
def rescue_lms(points,points_per_frame,lms_range = 50, return_points = False,floor_filter = 1.7):#sadly, the first lidar point has random yaw
    #each channel must be in order
    ele = np.pi*2 / points_per_frame
    yaw,d = xy2lms(points)
    if "0.9.8" in carla.__path__[0]  or "0.9.9.2" in carla.__path__[0]:
        on_floor_mask = points[:,2]>floor_filter#manually check!!! e.g. 1.8 is the z-location of lidar
    elif "0.9.11" in carla.__path__[0]:
        on_floor_mask = points[:,2]<-floor_filter
    idx = np.int32( np.mod(yaw,np.pi*2) / ele)#this once output 360?!!!!, because of it go out of precision of float e.g. float(359.99999999999) -> 360.0 
    ...
    if len(idx) > 0:
        ...
        for i in range(num_parts):
            d_rescue[i,idx[left[i]:right[i]]] = d[left[i]:right[i]] * (1-on_floor_mask[left[i]:right[i]]) + lms_range*on_floor_mask[left[i]:right[i]]
...
#        blueprint_lidar.set_attribute('sensor_tick', '0.03')#if this is below frequency, there will be missing frame
        if "0.9.8" in carla.__path__[0]:
            lidar = self.world.spawn_actor(blueprint_lidar,
                carla.Transform(carla.Location(x=0, y = 0, z=1.8), carla.Rotation(pitch=0,yaw = 90)),
                attach_to=self._parent)#when yaw = 0, the x-axis (zero degree) of local cordinates is vehicle's left. Anti-cloclwise. when yaw = 90, the x-axis will be vehicle's rear
            ...
            lidar = self.world.spawn_actor(blueprint_lidar,
                carla.Transform(carla.Location(x=-0.5, y = 0, z=1.8), carla.Rotation(pitch=0,yaw = 180)),#yaw = 180 means vehicle's rear
                attach_to=self._parent)
        elif "0.9.9.2" in carla.__path__[0]:#version update
            lidar = self.world.spawn_actor(blueprint_lidar,
                carla.Transform(carla.Location(x=0, y = 0, z=2.3), carla.Rotation(pitch=0,yaw = 90)),
                attach_to=self._parent)#this z is for tesla model 3
            
        self.sensor = lidar
        weak_self = weakref.ref(self) 
        self.sensor.listen(lambda lidar_frame: LidarSensor._parse_data(weak_self, lidar_frame,self.lidar_points_per_frame))
        
    def _parse_data(weak_self,lidar_frame,lidar_points_per_frame):
        self = weak_self()
        points = raw_data2points(lidar_frame)#(n,3)
        
        floor_filter = 2.1# manually trial and error
...
#        self.lidar_points = points
    ...
    def render(self, display):
        if self.lidar_points is not None:
            image_surface = draw_lidar_from_points(display, self.lidar_points, blend = False, blit = True)
