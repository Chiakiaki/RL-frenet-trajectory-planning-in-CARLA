...
class BoltzmannDistributionType(ProbabilityDistributionType):
    # remark: change from catagorical distribution type
    # changes: we must ensure that the number of catagories can be a TENSOR, not a fixed value
    # the 'proba distribution from latent' function in type class generate self._prob_distribution in policy
    # this is a link from policy_latent to distribution
    def __init__(self):
        """
        Now nothing. 
        """
    ...
    def probability_distribution_class(self):
        return BoltzmannDistributionV1

    def proba_distribution_from_something(self, goodness_n,grouping_mn):
#        raise NotImplementedError
#        pdparam = linear(pi_latent_vector, 'pi', self.n_cat_tensor, init_scale=init_scale, init_bias=init_bias)
#        q_values = linear(vf_latent_vector, 'q', self.n_cat_tensor, init_scale=init_scale, init_bias=init_bias)
        return BoltzmannDistributionV1(goodness_n, grouping_mn)

    def param_shape(self):
        return [self.n_cat_tensor]
...
#        self._action = None
        ...
        self._pdtype = BoltzmannDistributionType()
        self._proba_distribution = None
        
        """End of changes"""
    
    def _setup_init(self):
        #this need _policy_latent defined
        """
...
class FeedForwardBoltzmannDistributionPolicy(BoltzmannDistributionPolicy):
    ...
    # this part should be incharge of compute self._policy_latent and self._value_fn
    """
