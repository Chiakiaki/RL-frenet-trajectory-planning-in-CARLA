...
#            clipped_action = action
        ...
        mb_obs = np.asarray(mb_obs, dtype=self.obs.dtype).swapaxes(1, 0).reshape([-1] + list(self.env.observation_space.shape))
...
#        mb_actions = np.asarray(mb_actions, dtype=self.env.action_space.dtype).swapaxes(0, 1)
        ...
        a_label = np.asarray(a_label_episode,dtype = np.int32)
        prob_n = np.asarray(prob_episode,dtype = np.float32).reshape([-1])#flatten it
        all_a = np.asarray(all_a_episode,dtype = np.float32).reshape([-1] + list(d_action))
        num_a = np.asarray(num_a_list,dtype = np.int32)
